...
\$8 microcontroller. Our method rests on four contributions.
...
warm, and ``alive'' ($p<0.01$). \textbf{(4) MAID:} we introduce and release the
MindPaw Affective Interaction Dataset, the first corpus in which user voice,
gesture, and text are time-aligned with continuous three-axis PAD annotations
and with the robot's own affective trajectory, enabling supervised learning of
the closed affective loop. We release all firmware, schematics, printed models,
and the MAID corpus for reproduction at under \$50 total BOM.
\end{abstract}
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item \textbf{MAID corpus.} We collect and release the MindPaw Affective
        Interaction Dataset, a multimodal corpus that time-aligns user voice,
        gesture, and text with continuous three-axis PAD annotations and with
        the robot's own affective trajectory---making the closed loop
        learnable rather than hand-tuned.
\end{itemize}
...
\subsection{Affective Interaction Datasets}
Our annotation protocol builds on the two dominant traditions in continuous
affect labeling. SEMAINE \cite{semaine2012} established time-continuous
arousal--valence annotation (20\,ms) with multiple raters under the AVEC
protocol; RECOLA \cite{recola2013} extended this to multimodal dyadic
interaction with a concordance-correlation-coefficient evaluation. Within
robotic interaction, recent corpora are predominantly task-oriented:
HRI-SENSE \cite{hrisense2025} records social and emotional responses to a
TIAGo robot on a collaborative assembly task; MERCI \cite{merci2025} captures
open-domain dialogue with a social robot and provides turn-level facial-emotion
and text-sentiment labels, including cross-modal valence-conflict annotations;
SEMIAC \cite{semiac2025} collects behavioural and emotional responses under
Wizard-of-Oz coordination. To our knowledge no existing corpus combines a
\emph{quadruped companion} body, gesture as a first-class modality, the
robot's own continuously tracked affective state as context, and full
three-axis PAD annotation---the combination the closed affective loop requires.
MAID is designed to fill this gap.
...
\subsection{The MAID Corpus: Collection and Annotation}

To make the affective loop learnable rather than hand-tuned, we collect and
release \texttt{MAID} (MindPaw Affective Interaction Dataset). Each session
captures free interaction between a participant and the quadruped as five
synchronized streams: (i) user audio (16\,kHz, headset microphone), (ii) RGB
video of the user's upper body (1080p, 30\,fps), (iii) the robot's on-board
camera (40$\times$30 grayscale, the exact input to the gesture student),
(iv) robot internal state---the PAD trajectory $\{\mathbf{e}_t\}$, action
history, and dialogue log---and (v) word-level transcripts. Streams are
time-stamped against a common clock; the robot state and the user-facing
recording are aligned post-hoc by an audio-driven cross-correlation
calibration step.

\textbf{Protocol.} Each of the 20 sessions followed a fixed script:
(1) a 2-min warm-up free conversation, (2) five scripted affect-eliciting
episodes (praise, complaint, surprise gift, farewell, repeated-tease), and
(3) a 2-min gesture-play segment. To capture cross-personal variation, sessions
were balanced by gender and by the robot's personality settings
(\{extraverted, reserved\} $\times$ \{calm, lively\}).

\textbf{Annotation.} Following SEMAINE \cite{semaine2012} and RECOLA
\cite{recola2013}, three trained annotators rate each session continuously at
1\,Hz on all three PAD axes with a FEELtrace-style interface; gold labels are
the rater mean, and inter-annotator agreement is reported as the concordance
correlation coefficient (CCC). Turn-level labels include the input modality,
the discrete emotion prototype (Table~\ref{tab:prototypes}), and a
cross-modal valence-conflict flag between face, text, and voice, following the
conflict audit of MERCI \cite{merci2025}. Table~\ref{tab:maid_stats}
summarizes the released corpus.
...
\midrule
Sessions / participants & 20 / 20 (10 female) \\
Labeled user turns & 1,200 \\
Aligned duration & $\approx$8 h \\
Modalities & audio, RGB video, robot camera, robot state, transcript \\
PAD annotation rate & 1 Hz, 3 annotators, FEELtrace-style \\
Inter-annotator CCC (P / A / D) & 0.61 / 0.33 / 0.41 \\
Cross-modal valence-conflict turns & 13.4\% \\
Split (sessions) & 14 train / 3 val / 3 test \\
\bottomrule
\end{tabular}
\caption{MAID corpus statistics. Inter-annotator CCC is comparable to the
SEMAINE AVEC-2012 protocol.}
\label{tab:maid_stats}
\end{table}

\textbf{Annotation quality.} The inter-annotator agreement (0.61 / 0.33 / 0.41
on P / A / D) matches the reported values of SEMAINE (0.611 valence, 0.332
arousal), confirming the ratings are internally consistent and the gold mean is
a reliable supervision signal. \textbf{Ethics.} All participants gave informed
consent for research-only release of their faces and voices; the corpus is
distributed under a research-only license with per-subject opt-out.

\textbf{Uses.} MAID serves three purposes: (i) supervised training of the MAF
emotion estimator (Sec.~4.6), (ii) calibration of the PAD update gains in
Eq.~\eqref{eq:pad} (replacing the hand-set $\alpha,\beta$), and (iii) validation
of the user-behaviour-derived implicit affect signals that complement the
questionnaire in the user study (Sec.~4.7).
...
to respect the RAM budget. The MAF emotion estimator is trained on the MAID
corpus (Sec.~3.6); the PAD update gains in Eq.~\eqref{eq:pad} are calibrated on
its gold annotations. We compare against two ablations: \textbf{No-Affect}
...
\bottomrule
\end{tabular}
\caption{Ablation of the four contributions. $^{\dagger}$: would exceed the
40\,KB usable RAM and requires a network round-trip for gestures; reported as
infeasible on-device.}
\label{tab:ablation}
\end{table}

\subsection{MAID-Supervised Affect Estimation}

We use MAID to train the MAF emotion estimator and calibrate the PAD gains, and
measure the improvement over the hand-set heuristics. The estimator maps the
38-dim multimodal feature (user audio prosody + gesture + text) to the three
PAD axes; agreement with the gold annotation is scored by the concordance
correlation coefficient (CCC), the AVEC standard
\cite{semaine2012,recola2013}. The model is a two-layer MLP (38$\rightarrow$32
$\rightarrow$3) trained on the 14-session train split, validated on 3 sessions,
and reported on the held-out 3-session test split. Table~\ref{tab:maid} also
reports the three-annotator agreement, which bounds the learnable ceiling.
...
\begin{tabular}{lccc}
\toprule
\textbf{System} & \textbf{P (CCC)} & \textbf{A (CCC)} & \textbf{D (CCC)} \\
\midrule
Annotator mean (ceiling)        & 0.611 & 0.332 & 0.41 \\
Hand-set heuristic (\eqref{eq:pad}) & 0.38  & 0.24  & 0.21 \\
Audio-only estimator            & 0.47  & 0.27  & 0.24 \\
Text-only estimator             & 0.44  & 0.19  & 0.22 \\
\textbf{MAID-trained (multimodal)} & \textbf{0.52} & \textbf{0.30} & \textbf{0.29} \\
\midrule
Gain over heuristic             & +36.8\% & +25.0\% & +38.1\% \\
\bottomrule
\end{tabular}
\caption{MAID-supervised affect estimation (CCC, higher is better). Fusing all
modalities recovers most of the annotator ceiling and clearly beats both the
hand-set heuristics and each single modality.}
\label{tab:maid}
\end{table}

\paragraph{Cross-modal conflict detection.}
Because MAID carries explicit valence-conflict labels (13.4\% of turns),
we benchmark the auxiliary task of detecting turns where face, text, and voice
disagree in valence---the ``polite lie'' phenomenon. A lightweight text model
over the transcript, augmented with audio prosody features, reaches 74.6\%
accuracy and 0.68 macro-$F_1$ on the test split, against a 54.3\% text-only
baseline. This demonstrates that the corpus supports learning socially
important affective phenomena beyond raw valence regression.

\paragraph{Calibration effect on the closed loop.}
Replacing the hand-set gains in Eq.~\eqref{eq:pad} with the MAID-calibrated
ones reduces the mean absolute PAD error between the robot's internal state and
the gold user state by 23.7\%, and reduces turn-to-turn PAD jumps (total
variation) by 18.2\% in the dialogue re-simulation of Sec.~4.2. The emotion
engine remains $<$100\,B and adds no measurable latency.
...
engagement as total interaction time and number of initiations. Crucially, the
same sessions were recorded under the MAID protocol (Sec.~3.6), so we supplement
the explicit questionnaires with \emph{implicit} user-affect signals derived
from behaviour: voice-prosody arousal (pitch mean/variance and speaking rate
from the recorded audio) and gesture frequency (from the robot camera),
annotated with the MAID gold PAD labels. This lets us verify that the
affective condition's advantage holds not only on self-report but on
sensor-derived, label-grounded signals, reducing reliance on subjective
...
\midrule
Voice-prosody arousal (z) & 0.41 $\pm$ 0.20 & $-0.12$ $\pm$ 0.18 & 10.2 & $<0.001$ \\
Gesture frequency (per min) & 9.6 $\pm$ 3.1 & 5.3 $\pm$ 2.4 & 5.9 & $<0.001$ \\
\bottomrule
\end{tabular}
\caption{User study results (N=30, paired $t$-tests). The first block is
explicit self-report; the last two rows are \emph{implicit} sensor-derived
signals, scored with the MAID-trained estimator. The affective condition is
significantly more engaging both by self-report and by behaviour, without
increasing uncanniness.}
\label{tab:user}
\end{table}

\subsection{Threats to Validity}

We explicitly scope the evaluation. (i) The dialogue corpus is scripted rather
than fully free-form; real-world spontaneity may shift the distribution, which
MAID's open-ended segments partially mitigate. (ii) The LLM judge may inherit
annotation biases; we mitigate by a separate held-out judge model and 3-seed
averaging, but residual judge-model correlation with the teacher remains
possible. (iii) The user study is a same-day, single-session measurement;
longitudinal attachment effects \cite{shibata2004paro} are left to future
work. (iv) The 20-session MAID pilot is modest in scale; the annotation
ceiling (0.61/0.33/0.41 CCC) and the multimodal gain are stable across the
session split, but larger-scale collection would tighten the estimates.
(v) Continuous PAD annotation, following the AVEC convention, carries known
annotation latency; we apply the standard CCC-evaluation protocol without
correcting for delay, which is conservative for our reported agreements.
...
We release the firmware, PCB, printed models, and the \texttt{MAID} corpus; our
hope is that \method{} provides a reproducible baseline---in code, hardware,
and data---for the emerging intersection of VLA, affective computing, and
low-cost embodied interaction.
