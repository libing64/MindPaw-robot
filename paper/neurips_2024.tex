...
To make the affective loop learnable rather than hand-tuned, we collect the
\texttt{MAID} (MindPaw Affective Interaction Dataset). Each session captures
free interaction with the quadruped as synchronized streams of:
(i) user audio (16\,kHz headset), (ii) RGB video of the user's upper body,
(iii) the robot's on-board camera, (iv) robot internal state (the PAD
trajectory $\{\mathbf{e}_t\}$, action history, dialogue log), and (v) time-stamped
transcripts.

\textbf{Annotation.} Following SEMAINE \cite{semaine2012} and RECOLA
\cite{recola2013}, three trained annotators rate each session continuously
(1\,Hz) on all three PAD axes with a FEELtrace-style interface; gold labels are
the rater mean and annotator agreement is reported as the concordance
correlation coefficient, as in the AVEC protocols. Turn-level labels include
the input modality, the discrete emotion prototype
(Table~\ref{tab:prototypes}), and cross-modal valence conflicts between face,
text, and voice, following the conflict audit of MERCI \cite{merci2025}.

\textbf{Statistics.} Initial release: 20 sessions, 1,200 labelled user turns,
$\approx$8 hours of aligned audio--video--robot-state, balanced by user gender
and by robot personality settings. \textbf{Ethics.} All participants gave
informed consent for research-only release of their faces and voices; the corpus
is distributed under a research-only license.

\textbf{Uses.} MAID serves three purposes: (i) supervised training of the MAF
emotion estimator, (ii) calibration of the PAD update rule in
Eq.~\eqref{eq:pad} (replacing hand-set $\alpha,\beta$), and (iii) validation of
the user-behaviour-derived implicit affect signals that complement the
questionnaire in our user study (Sec.~4.5).
...
38-dim multimodal feature (user audio prosody + gesture + text) to the
three PAD axes; agreement with the gold annotation is scored by the
concordance correlation coefficient (CCC), the AVEC standard
\cite{semaine2012,recola2013}. Table~\ref{tab:maid} also reports the
three-annotator agreement, which bounds the learnable ceiling.
...
Hand-set heuristic (Eq.~3)   & 0.38  & 0.24  & 0.21 \\
\textbf{MAID-trained estimator} & \textbf{0.52} & \textbf{0.30} & \textbf{0.29} \\
\midrule
Gain over heuristic          & +36.8\% & +25.0\% & +38.1\% \\
\bottomrule
\end{tabular}
\caption{MAID-supervised affect estimation (CCC, higher is better). The
MAID-trained estimator approaches the annotator ceiling and clearly beats the
hand-set PAD heuristics, validating the data-driven calibration of
Eq.~\eqref{eq:pad}.}
\label{tab:maid}
\end{table}
